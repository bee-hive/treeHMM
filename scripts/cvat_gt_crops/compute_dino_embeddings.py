"""
Step 2: Compute DINOv2 embeddings of dino_patch_px x dino_patch_px crops centered
on T cell track centroids.
...
/ yaml.  The tiny pure-numpy `get_RGB_image_with_nuclei` helper is inlined here
(copied from MarsonImagingPipeline PipelineUtils).

For each crop:
  - read the per-crop crop.tiff  (T, H, W, 2): channel 0 = RFP nuclei, 1 = phase
  - build one RGB per full crop frame (consistent contrast)
  - extract a dino_patch_px patch centered on each T-cell centroid (edge padded)
  - run facebook/dinov2-base (CLS / pooler output, 768-d) on the batched patches

Saved per crop (into {output_base_dir}/{crop}/):
    t_cell_dino_raw.npy        (T, N, 768)  raw embeddings; NaN where no valid patch
    t_cell_dino_valid_mask.npy (T, N)       True where a real patch was embedded

Column ordering matches t_cell_cell_ids.npy / t_cell_centroids.npy from Step 1.
...
for crop in crop_ids:
    ...
    crop_path = os.path.join(cvat_base_dir, well, crop, "crop.tiff")
    crop_img = np.array(tifffile.imread(crop_path))  # (T, H, W, 2): ch0=RFP nuclei, ch1=phase
    T = crop_img.shape[0]

    out_dir = os.path.join(out_base_dir, crop)
    t_cell_ids = np.load(os.path.join(out_dir, "t_cell_cell_ids.npy"))
    centroids = np.load(os.path.join(out_dir, "t_cell_centroids.npy"))  # (T, N, 2)
    N = len(t_cell_ids)
    ...
    np.save(os.path.join(out_dir, "t_cell_dino_raw.npy"), raw)
    np.save(os.path.join(out_dir, "t_cell_dino_valid_mask.npy"), valid)

#!/usr/bin/env bash
# ============================================================
# Cancer-PHASE DINOv2 tree-AR-HMM pipeline
# ============================================================
...
#   Step 1  calculate_cancer_phase_emissions.py OccidentAnalysis  (StatUtils features)
#   Step 2  compute_dino_embeddings.py          cs229Dino         (self-contained, DINOv2)
#   Step 3  reduce_dino_pca.py                  cs229Dino         (PCA)
#   Step 4  fit_cancer_phase_arhmm.py           treeHMM_env       (JAX / model fit)
#   Step 5  create_cancer_phase_overlay_videos.py OccidentAnalysis (video render)
#
# NOTE: Step 0 regenerates the type-separated tracks. Since this pipeline reuses
# the cancer_dino_hmm type_sep_tracks dir by default, Step 0 can be skipped if
# those tracks already exist.
#
# Usage:
#   cd scripts/cancer_phase_dino_hmm
...
run "$FIT_ENV" python fit_cancer_phase_arhmm.py                 # Step 4
run "$OCCIDENT_ENV" python create_cancer_phase_overlay_videos.py # Step 5

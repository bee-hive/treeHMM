"""
Step 4: Jointly fit the tree-AR-HMM across cancer PHASE cells in all crops.

Cancer cells are defined by their phase masks (channel 1 of the type-separated
tracks; each CVAT cancer track == one cancer cell).  Emissions = z-scored
non-DINO features (model_features, e.g. area + dilated_t_cell_neighbors)
concatenated with the top-`n_dino_pcs` PCA'd DINOv2 embeddings (if use_dino).
...
Usage (treeHMM_env):
    conda run -n treeHMM_env python fit_cancer_phase_arhmm.py
"""
...
def load_cancer_phase_tracks(crop):
    ...
    tr = np.asarray(tifffile.imread(
        os.path.join(type_sep_tracks_dir, crop, "tracks.tiff")))
    return tr[..., 1]


# ============================================================
# Step 1: Build per-crop masks from the cancer phase tracks
# ============================================================
print("=" * 60)
print("Step 1: Building data dicts (cancer = phase tracks, channel 1)")
print("=" * 60)

all_data_list, crop_num_cells, crop_labels = [], [], []

for crop_idx, crop in enumerate(crop_ids):
    cancer_tracks = load_cancer_phase_tracks(crop)
    ...
    assert np.array_equal(cancer_cell_ids, derived), (
        f"cancer_cell_ids.npy disagrees with phase tracks for {crop} "
    ...
    for t in range(T):
        frame_ids = np.unique(cancer_tracks[t][cancer_tracks[t] > 0])
    ...
    if allow_divisions:
        # The phase channel carries the CVAT track IDs that key graph.pkl directly,
        # so divisions could be wired without the CVAT->nucleus overlap mapping that
        # cancer_dino_hmm/divisions_utils.py performs. That wiring is not implemented
        # for this pipeline yet.
        raise NotImplementedError(
            "allow_divisions is not yet supported in cancer_phase_dino_hmm; "
...
axes[1].set_title('State Assignments (argmax smoothed_probs) - cancer phase cells')
...
# Show non-DINO features in their real units (invert the z-score).
if standardize_nondino:
...
_discrete = {'t_cell_neighbors_20px', 'dilated_t_cell_neighbors'}
...
plt.suptitle('Fraction of cancer phase cells per state over time')
...
axes[1].set_xlabel('Time'); axes[1].set_title('T cells within 20px of cancer phase cell - All Crops')

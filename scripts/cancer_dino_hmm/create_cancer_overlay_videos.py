"""
Step 5: Create HMM overlay videos for the cancer-cell crops.

For each crop:
  1) Load the crop background image, Caliban nuclei tracks (cancer cells), and the
     regenerated T-cell tracks (secondary layer)
  2) Reconstruct the same min_t cell filter used during fitting
  3) Load per-crop cancer_state_assignments.npy
  4) Color each cancer nucleus by its HMM state (warmup frames in grey)
  5) Render an mp4

Usage (OccidentAnalysis):
    conda run -n OccidentAnalysis python create_cancer_overlay_videos.py
"""
...
for crop in crop_ids:
    ...
    # 1. Load background, cancer nuclei tracks, T-cell tracks
    raw_tiff = tifffile.imread(os.path.join(cvat_base_dir, well_id, crop, 'crop.tiff'))
    cancer_tracks = load_nuclei_tracks(crop)                       # (T,H,W) nucleus IDs == cancer cells
